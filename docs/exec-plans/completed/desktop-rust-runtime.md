...
### Rust / Tauri
- [x] 新增 runtime supervisor，自动拉起本地 Python sidecar
- [x] 新增 runtime 状态命令与事件广播
- [x] 新增 macOS Keychain 会话存取命令
- [x] 新增文件选择 / Reveal / 通知命令

### 前端
- [x] 新增 `desktopBridge` 与 runtime store
- [x] 移除 `baseUrl` 启动门槛，改为 runtime 启动门槛
- [x] 登录与登出改为走安全会话存储
- [x] 增加 runtime 启动页和 diagnostics 视图
- [x] 设置页新增桌面运行时信息与操作入口

### 后端
- [x] 新增 desktop domain/router/service
- [x] 新增 desktop runtime / jobs / watch-folders 基础接口
- [x] FastAPI startup 支持 stdout runtime 事件
- [x] 为 desktop profile 补充最小配置与兼容逻辑

### 测试
- [x] Rust 单元测试：状态解析、stdout 事件解析
- [x] Python 单元测试：desktop service / watch-folder registry
- [x] 前端构建与测试：desktop 启动与 runtime diagnostics
...
- [x] 桌面端启动时自动探测并启动本地 sidecar
- [x] 不再要求用户先填写 server URL 才能进入登录页
- [x] 登录凭证不再持久化到浏览器 localStorage
- [x] runtime 启动失败时显示 diagnostics，而不是卡死在 setup 页
- [x] `cargo test`、`pytest`、`pnpm build` 至少覆盖本轮新增能力
...
- 2026-04-17: 桌面知识库导入优先走“原生选文件 + 本地路径导入”而不是继续沿用浏览器 file input。
