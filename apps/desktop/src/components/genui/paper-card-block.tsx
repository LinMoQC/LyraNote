...
function PaperCardBlockInner({ code, isStreaming }: { code: string; isStreaming?: boolean }) {
  ...
  if (!data) return <pre className="my-2 overflow-x-auto rounded-xl bg-white/[0.06] p-3 font-mono text-xs leading-5"><code>{code}</code></pre>

  return (
    <div className="my-3 rounded-xl border border-white/[0.08] bg-white/[0.03] p-4">
      <div className="flex items-start justify-between gap-3">
        <h4 className="text-sm font-semibold leading-snug text-white/90">{data.title}</h4>
        {data.citations != null && (
      ...
      {data.abstract && (
        <p className="mt-2 line-clamp-3 text-xs leading-relaxed text-white/70">{data.abstract}</p>
      )}
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        ...
        {data.doi && (
          <a
            ...
            className="inline-flex items-center gap-1 rounded-md bg-white/[0.06] px-2 py-1 text-[10px] text-white/40 transition-colors hover:bg-white/[0.1] hover:text-white/80"
          >
