...
function HeatmapBlockInner({ code, isStreaming }: { code: string; isStreaming?: boolean }) {
  ...
  if (isStreaming) {
    return (
      <div className="my-3 flex h-40 items-center justify-center rounded-xl border border-white/[0.08] bg-white/[0.03] text-xs text-white/40">
        {t("heatmapStreaming")}
      </div>
    )
  }
  ...
  if (!data || !Array.isArray(data.data)) return <pre className="my-2 overflow-x-auto rounded-xl bg-white/[0.06] p-3 font-mono text-xs leading-5"><code>{code}</code></pre>
  ...
  return (
    <div className="my-3 rounded-xl border border-white/[0.08] bg-white/[0.03] p-4">
      {data.title && <p className="mb-2 text-sm font-medium text-white/80">{data.title}</p>}
