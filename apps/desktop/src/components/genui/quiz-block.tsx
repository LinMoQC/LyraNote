...
function QuizBlockInner({ code, isStreaming }: { code: string; isStreaming?: boolean }) {
  ...
  if (!raw || !Array.isArray(qArr)) return <pre className="my-2 overflow-x-auto rounded-xl bg-white/[0.06] p-3 font-mono text-xs leading-5"><code>{code}</code></pre>
  ...
  return (
    <div className="my-3 space-y-4 rounded-xl border border-white/[0.08] bg-white/[0.03] p-4">
      {data.title && (
        <div className="flex items-center justify-between">
          <h4 className="text-sm font-semibold text-white/90">{data.title}</h4>
          {totalAnswered > 0 && (
            <span className="text-xs text-white/40">{totalAnswered}/{data.questions.length}</span>
          )}
        </div>
      )}
        ...
        return (
          <div key={qi} className="space-y-2">
            <p className="text-sm font-medium text-white/80">
              {data.questions.length > 1 && <span className="text-white/35">{qi + 1}. </span>}
              {q.q}
            </p>
              ...
              {q.options.map((opt, oi) => {
                ...
                return (
                  ...
                  >
                    ...
                    {!answered && <span className="flex h-4 w-4 shrink-0 items-center justify-center rounded-full border border-white/20 text-[10px] text-white/35">{String.fromCharCode(65 + oi)}</span>}
                    <span>{opt}</span>
                  </button>
                )
              })}
