...
function MatrixBlockInner({ code, isStreaming }: { code: string; isStreaming?: boolean }) {
  ...
  if (!data || !Array.isArray(data.criteria) || !Array.isArray(data.options)) return <pre className="my-2 overflow-x-auto rounded-xl bg-white/[0.06] p-3 font-mono text-xs leading-5"><code>{code}</code></pre>
    ...
    <div className="my-3 overflow-x-auto rounded-xl border border-white/[0.08]">
      <table className="w-full border-collapse text-sm">
        <thead className="bg-white/[0.04]">
          <tr>
            <th className="whitespace-nowrap px-3 py-2 text-left text-xs font-semibold text-white/70">{t("matrixOption")}</th>
            {data.criteria.map((c, i) => (
              <th key={i} className="whitespace-nowrap px-3 py-2 text-center text-xs font-semibold text-white/70">
                {c}
                {weights[i] != null && (
                  <span className="ml-1 text-[10px] text-white/30">({(weights[i] * 100).toFixed(0)}%)</span>
                )}
              </th>
            ))}
            <th className="whitespace-nowrap px-3 py-2 text-center text-xs font-semibold text-white/70">{t("matrixTotal")}</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-white/[0.06]">
          {data.options.map((opt, oi) => (
            <tr key={oi} className="transition-colors hover:bg-white/[0.02]">
              <td className="whitespace-nowrap px-3 py-2 font-medium text-white/80">
                {opt.name}
                {totals[oi] === maxTotal && (
              ...
              <td className="px-3 py-2 text-center">
                <span className={cn(
                  ...
                  totals[oi] === maxTotal ? "bg-amber-500/20 text-amber-300" : "bg-white/[0.06] text-white/70"
                )}>
                  {totals[oi].toFixed(1)}
                </span>
              </td>
