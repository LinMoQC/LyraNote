...
function TableBlockInner({ code, isStreaming }: { code: string; isStreaming?: boolean }) {
  ...
  if (!raw || !Array.isArray(rawRows) || rawRows.length === 0) return <pre className="my-2 overflow-x-auto rounded-xl bg-white/[0.06] p-3 font-mono text-xs leading-5"><code>{code}</code></pre>
  ...
  if (Array.isArray(rawColumns) && rawColumns.length > 0) {
    ...
    if (typeof first === "object" && first !== null && !Array.isArray(first)) {
      ...
      return <pre className="my-2 overflow-x-auto rounded-xl bg-white/[0.06] p-3 font-mono text-xs leading-5"><code>{code}</code></pre>
    }
  }
          ...
          <tr>
            {columns.map((col, i) => (
              <th
                ...
                className="cursor-pointer select-none whitespace-nowrap px-3 py-2 text-left text-xs font-semibold text-white/70 transition-colors hover:text-white/90"
              >
        ...
        <tbody className="divide-y divide-white/[0.06]">
          {sortedRows.map((row, ri) => (
            <tr key={ri} className="transition-colors hover:bg-white/[0.02]">
              {row.map((cell, ci) => (
                <td key={ci} className="px-3 py-2 text-white/80" title={String(cell)}>
