    ...
    <div className="flex-1 rounded-xl border border-white/[0.08] bg-white/[0.03] p-4">
      <div className="flex items-center gap-2.5">
        ...
        <div className="min-w-0">
          <h4 className="text-[13px] font-semibold leading-tight text-white/90">{card.title}</h4>
          {card.subtitle && (
            <p className="mt-0.5 text-[10px] text-white/35">{card.subtitle}</p>
          )}
        </div>
      </div>
          ...
          return (
            <div key={i} className="flex items-start gap-2.5 py-2 first:pt-0 last:pb-0">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className={`text-xs leading-relaxed text-white/75 ${label ? "mt-0.5" : ""}`}>{value}</p>
              </div>
            </div>
          )
        })}
...
function CardBlockInner({ code, isStreaming }: { code: string; isStreaming?: boolean }) {
  ...
  if (!parsed) return <pre className="my-2 overflow-x-auto rounded-xl bg-white/[0.06] p-3 font-mono text-xs leading-5"><code>{code}</code></pre>
  const cards = Array.isArray(parsed) ? parsed : [parsed]
  if (cards.length === 0 || !cards[0]?.items) return <pre className="my-2 overflow-x-auto rounded-xl bg-white/[0.06] p-3 font-mono text-xs leading-5"><code>{code}</code></pre>
