...
export function buildMarkdownComponents(opts: MarkdownComponentsOpts) {
  ...
  return {
    ...
    strong: ({ children }: React.HTMLAttributes<HTMLElement>) => <strong className="font-semibold text-white/90">{processChildren(children, citations)}</strong>,
    ...
    h1: ({ children }: React.HTMLAttributes<HTMLHeadingElement>) => <h1 className="mb-3 mt-6 text-xl font-bold text-white/90">{processChildren(children, citations)}</h1>,
    h2: ({ children }: React.HTMLAttributes<HTMLHeadingElement>) => <h2 className="mb-2.5 mt-5 text-lg font-bold text-white/90">{processChildren(children, citations)}</h2>,
    h3: ({ children }: React.HTMLAttributes<HTMLHeadingElement>) => <h3 className="mb-2 mt-4 text-base font-semibold text-white/90">{processChildren(children, citations)}</h3>,
    h4: ({ children }: React.HTMLAttributes<HTMLHeadingElement>) => <h4 className="mb-1.5 mt-3 text-sm font-semibold text-white/90">{processChildren(children, citations)}</h4>,
    h5: ({ children }: React.HTMLAttributes<HTMLHeadingElement>) => <h5 className="mb-1 mt-2.5 text-[13px] font-semibold text-white/80">{processChildren(children, citations)}</h5>,
    blockquote: ({ children }: React.HTMLAttributes<HTMLElement>) => <blockquote className="my-1.5 border-l-2 border-primary/40 pl-3 text-white/70">{processChildren(children, citations)}</blockquote>,
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    code: ({ children, className, ...props }: any) => {
      const isInline = !("data-language" in props) && !className
      if (isInline) return <code className="rounded bg-white/[0.07] px-1.5 py-0.5 font-mono text-xs text-white/90">{children}</code>
    ...
    th: ({ children }: React.HTMLAttributes<HTMLTableCellElement>) => <th className="px-3 py-2 text-left text-xs font-semibold text-white/70">{children}</th>,
    td: ({ children }: React.HTMLAttributes<HTMLTableCellElement>) => <td className="px-3 py-2 text-white/80">{children}</td>,
