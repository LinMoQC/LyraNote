...
function SortableCard({ card }: { card: KanbanCard }) {
  ...
  return (
    ...
    >
      <p className="font-medium text-foreground/80">{card.title}</p>
      {card.tag && (
...
function KanbanBlockInner({ code, isStreaming }: { code: string; isStreaming?: boolean }) {
  ...
  return (
    <div className="my-3 flex gap-3 overflow-x-auto rounded-xl border border-border/40 bg-muted/10 p-4">
        ...
        <SortableContext items={allCardIds} strategy={verticalListSortingStrategy}>
          {columns.map((col) => (
            <div key={col.id} className="flex w-48 shrink-0 flex-col gap-2">
              <h5 className="text-xs font-semibold text-foreground/70">
                {col.title}
                <span className="ml-1 text-muted-foreground/40">({col.cards.length})</span>
              </h5>
