...
function TimelineBlockInner({ code, isStreaming }: { code: string; isStreaming?: boolean }) {
  ...
  if (!raw || !Array.isArray(eventArr)) return <pre className="my-2 overflow-x-auto rounded-xl bg-accent/60 p-3 font-mono text-xs leading-5"><code>{code}</code></pre>

  // eslint-disable-next-line react-hooks/rules-of-hooks
  const events = useMemo(() => eventArr.map(normalizeEvent), [eventArr])

  return (
    <div className="my-3 overflow-hidden rounded-xl border border-border/40 bg-gradient-to-b from-muted/15 to-muted/5 p-5">
      {raw.title && (
        <h4 className="mb-5 flex items-center gap-2 text-sm font-semibold text-foreground/90">
              ...
              <div className={cn(
                ...
                <div className="flex items-center gap-2">
                  ...
                  <h5 className={cn(
                    ...
                    event.highlight ? "text-indigo-200" : "text-foreground/80"
                  )}>
                    {event.title}
                  </h5>
                </div>
                {event.desc && (
                  <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground/55">{event.desc}</p>
                )}
              </div>
            </div>
          )
