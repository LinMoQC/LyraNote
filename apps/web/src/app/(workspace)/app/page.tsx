...
export default async function AppHomePage() {
  ...
  return (
    <div className="flex min-h-full flex-col items-center justify-center border-border/40 px-4 dark:border md:px-8">
      ...
      {/* Mobile layout */}
      <div className="flex w-full flex-1 flex-col items-center justify-center px-6 pb-44 pt-10 md:hidden">
        <div className="mx-auto flex w-full max-w-sm items-center justify-center">
          <HomeHero greeting={greeting} tagline={tagline} align="center" />
        </div>
      </div>

      {/* Mobile fixed bottom input */}
      <div className="fixed inset-x-0 bottom-0 z-30 px-4 pb-[calc(env(safe-area-inset-bottom)+12px)] pt-3 md:hidden">
        <div className="mx-auto flex w-full max-w-[430px] flex-col gap-3">
          <HomeContent
            showQa
            showSuggestions
            qaShowHint={false}
            suggestionsVariant="hero"
