...
async def retrieve_chunks(
    ...
    *,
    ...
    # If variants were pre-generated by the caller (rag_agent_node), skip the
    # LLM call and embed the primary variant directly.
    if _precomputed_variants is not None:
        variants = _precomputed_variants
        primary = variants[0]
        embed_result = await embed_query(primary)
        query_vec = embed_result if not isinstance(embed_result, Exception) else None
    else:
